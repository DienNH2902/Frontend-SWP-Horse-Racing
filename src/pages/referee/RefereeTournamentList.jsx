...
export default function RefereeTournamentList() {
  ...
  const [raceCourseName, setRaceCourseName] = useState("");
  ...
  async function handleViewTournament(tournament) {
    try {
      ...
      const races = await getRacesByTournament(tournament._id);
  ...
  async function handleViewRace(raceId) {
    try {
      ...
      if (race?.raceCourseId) {
        try {
          const course = await getRaceCourseById(race.raceCourseId);

          courseName = course?.name || course?.data?.name || "-";
        } catch (error) {
          console.error(error);
        }
      }
      ...
      const participantDetails = await Promise.all(
        participants.map(async (participant) => {
          ...
          try {
            const horse = await getHorseById(participant.horseId);
          ...
          try {
            const jockey = await getUserById(participant.jockeyId);
          ...
          return {
            gateNumber: participant.gateNumber,
            horseName,
            horseImage,
            jockeyName,
          };
      ...
      setRaceParticipants(participantDetails);
  ...
  return (
    <>
      <div className="referee-dashboard-page">
        ...
        >
          <Card className="dashboard-hero">
            <Row align="middle" justify="space-between" gutter={[24, 24]}>
              <Col xs={24} sm={24} md={16} lg={16}>
                <div className="dashboard-badge">GOLDEN HOOF RACING SYSTEM</div>

                <Typography.Title level={1} className="dashboard-title">
                  Tournament
                  <span className="dashboard-title-highlight"> Explorer</span>
                </Typography.Title>

                <Typography.Paragraph className="dashboard-subtitle">
                  Browse tournaments, inspect races, review participants, and
                  monitor every competition.
          ...
          <Card
            ...
            title={
              <span className="dashboard-card-title">Tournament List</span>
            }
            extra={
              <Space>
            ...
            extra={
              <Typography.Text className="dashboard-card-extra">
                ...
                <div className="dashboard-filter-bar">
                  ...
                  <ConfigProvider
                    theme={{
                      components: {
                        Select: {
                          colorText: "white", // Màu chữ của item được chọn
                          colorTextPlaceholder: "white", // Màu chữ của placeholder (nếu muốn)
                        },
                      },
                    }}
                  >
        ...
        {selectedTournament && (
          <>
            ...
            {tournamentLoading ? (
              ...
              <List
                header={
                  <Typography.Title level={5}>Race List</Typography.Title>
                }
                bordered
                dataSource={selectedTournament.races || []}
                renderItem={(race) => (
                  <List.Item
                    actions={[
                      <Button
                        key="view"
                        className="dashboard-table-btn"
                        onClick={() => handleViewRace(race._id)}
                      >
                        View Race Detail
                      </Button>,
                    ]}
                  >
                    <List.Item.Meta
                      title={
                        <Typography.Text className="race-title">
                          {race.name}
                        </Typography.Text>
                      }
                      description={
                        <Space direction="vertical" size={4}>
                          <Tag color={statusColor[status]}>{race.status}</Tag>

                          <Text className="race-round">Round {race.round}</Text>
                        </Space>
                      }
                    />
                  </List.Item>
      ...
      <Modal
        className="dashboard-modal"
        open={raceModalOpen}
        onCancel={() => setRaceModalOpen(false)}
        footer={null}
        width={1100}
        style={{
          maxWidth: "95vw",
          ...
          selectedRace && (
            <>
              ...
              <div className="dashboard-section-header">
                ...
                <Tag color="cyan">{raceParticipants.length} Horses</Tag>
              </div>
              <div className="table-responsive">
                <Table
                  scroll={{ x: 600 }}
                  ...
                  size="large"
                  rowKey={(record) => record.gateNumber}
                  pagination={false}
                  dataSource={raceParticipants}
                  columns={[
                    {
                      title: "Gate",
                      dataIndex: "gateNumber",
                      render: (gate) => <Tag color="cyan">Gate {gate}</Tag>,
                    },
